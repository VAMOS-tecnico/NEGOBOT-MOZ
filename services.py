...
# ---------------------------
# Gestão de Histórico de Conversas (Corrigido para evitar erro de argumentos)
# ---------------------------
def get_chat_history(empresa_id: str, phone_number: str = None, limit: int = 10):
    """
    Busca o histórico de mensagens de uma conversa.
    Aceita 1 argumento (phone_number) ou 2 argumentos (empresa_id, phone_number).
    """
    try:
        db = get_db()
        if phone_number is None:
            phone_number = empresa_id
...
def save_chat_history(empresa_id: str, phone_number: str = None, role: str = "user", text: str = ""):
    """
    Guarda uma mensagem no histórico do Firestore.
    Aceita parâmetros flexíveis para evitar falhas de chamada.
    """
    try:
        db = get_db()
        if phone_number is None or role in ["user", "assistant", "atendente"] and text == "":
            # Caso tenha sido chamado como save_chat_history(phone_number, role, text)
            text = role
            role = phone_number or "user"
            phone_number = empresa_id
...
def send_whatsapp(to, text, instance_name=None):
    ...
    try:
        ...
        requests.post(f"{EVOLUTION_API_URL}/chat/sendPresence/{instance_name}", headers=headers, json={"number": clean_to, "presence": "composing"}, timeout=5)
        time.sleep(0.5)
        
        # Enviar mensagem de texto
        res = requests.post(f"{EVOLUTION_API_URL}/message/sendText/{instance_name}", headers=headers, json={"number": clean_to, "text": text}, timeout=10)
...
def handle_onboarding_from_message(phone_number: str, message_text: str, document_message: dict = None, instance_name: str = None):
    ...
    if empresa_doc is None:
        ...
        send_whatsapp(phone_number, "Olá! Para configurar o seu assistente, envie os dados do negócio (serviços, preços, horário) ou um ficheiro PDF/Excel.", instance_name=instance_name)
        append_empresa_history(empresa_id, {"event": "onboarding_requested", "timestamp": agora})
        return empresa_id
    ...
    if document_message:
        ...
        if updates:
            ...
            send_whatsapp(phone_number, "✅ Dados da empresa atualizados com sucesso!", instance_name=instance_name)
...
def _process(data):
    try:
        ...
        if image_message:
            ...
            if url_imagem:
                send_whatsapp(phone_number, "👁️ *A analisar a imagem/documento...*", instance_name=nome_instancia_atual)
                analise_foto = analisar_imagem_groq(url_imagem, instrucao=caption or "Extraia as informações desta imagem.")
                if analise_foto:
