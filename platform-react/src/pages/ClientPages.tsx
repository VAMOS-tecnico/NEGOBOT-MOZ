...
import { AlertCircle, BarChart3, Bot, CheckCircle2, CircleDollarSign, FileUp, LifeBuoy, Loader2, MessageCircle, Pause, Play, Plus, QrCode, RefreshCw, Send, Smartphone, Users, Video, XCircle } from "lucide-react";
import { api, type AssistantSettings, type Campaign, type CampaignTemplate, type ClientPlan, type Contact, type Conversation, type DeliveryMetrics, type IntegrationStatus, type LemonSqueezyStatus, type PaymentRecord, type Plan, type SupportTicket, type TeamMember, type TenantMetrics, type VideoJob } from "../lib/api";
...
export function BillingPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setPlan(current); setPlans(catalog.plans || []); setPayments(history.payments || []); setLemonStatus(online);
  ...
  async function verify(event: FormEvent) {
    ...
    try { const result = await api.client.verifyPayment(messageText, clientPhone); setNotice(result.response); setMessageText(""); await load(); }
  ...
  return <div className="content-stack">
    ...
    {lemonStatus?.configured && <section className="payment-instruction"><div className="quick-icon"><CircleDollarSign size={20} /></div><div><strong>Pagamento online com Lemon Squeezy</strong><p>Usa cartão, PayPal ou outro método apresentado no checkout. A subscrição só ativa o plano depois da confirmação segura do webhook.</p></div></section>}
    <section className="payment-instruction"><div className="quick-icon"><Smartphone size={20} /></div><div><strong>Pagamento manual por M-Pesa</strong><p>Transfere para <b>{mpesaNumber}</b>, em nome de <b>{mpesaName}</b>. Depois cola abaixo o SMS completo ou o ID da transação. O AutoPay compara a transação recebida antes de ativar o plano.</p></div></section>
    ...
    <section className="plan-grid">{busy ? <LoadingBox /> : plans.map((item) => <article className={`plan-card ${item.id === plan?.plan ? "selected" : ""}`} key={item.id}><span className="eyebrow">{item.name}</span><strong>{item.price_mt} MT</strong><small>{item.validity_days} dias</small><div className="benefits">{item.benefits.map((benefit) => <span key={benefit}><CheckIcon />{benefit}</span>)}</div><button className="secondary-button" type="button" onClick={() => setNotice(item.id === plan?.plan ? "Este é o teu plano atual." : `Plano ${item.name} selecionado. Faz a transferência de ${item.price_mt} MT e envia o SMS acima.`)}>{item.id === plan?.plan ? "Plano atual" : "Escolher por M-Pesa"}</button>{lemonStatus?.configured && lemonStatus.plans[item.id] && <button className="primary-button" type="button" disabled={checkoutPlan === item.id} onClick={() => void checkout(item.id)}>{checkoutPlan === item.id ? "A abrir checkout..." : "Pagar online"}</button>}</article>)}</section>
  </div>;
}
