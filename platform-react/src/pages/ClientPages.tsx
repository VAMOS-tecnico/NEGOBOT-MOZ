...
import { api, type AssistantSettings, type Campaign, type CampaignTemplate, type ClientPlan, type Contact, type Conversation, type IntegrationStatus, type Plan, type TeamMember } from "../lib/api";
...
export function BillingPage() {
  const [plan, setPlan] = useState<ClientPlan | null>(null); const [plans, setPlans] = useState<Plan[]>([]); const [mpesaNumber, setMpesaNumber] = useState("855000929"); const [mpesaName, setMpesaName] = useState("Abel Francisco"); const [messageText, setMessageText] = useState(""); const [clientPhone, setClientPhone] = useState(""); const [busy, setBusy] = useState(true); const [verifying, setVerifying] = useState(false); const [error, setError] = useState(""); const [notice, setNotice] = useState("");
  async function load() { setBusy(true); try { const [current, catalog] = await Promise.all([api.client.plan(), api.client.plans()]); setPlan(current); setPlans(catalog.plans || []); if (catalog.mpesa_number) setMpesaNumber(catalog.mpesa_number); if (catalog.mpesa_name) setMpesaName(catalog.mpesa_name); } catch (reason) { setError(reason instanceof Error ? reason.message : "Não foi possível carregar os planos."); } finally { setBusy(false); } }
  useEffect(() => { void load(); }, []);
