...
from services.business_logic import checar_timeout_atendimento_humano
from services.config import NUMERO_ASSISTANTE, EVOLUTION_INSTANCE_NAME, EVOLUTION_API_URL, EVOLUTION_API_KEY, TIMEOUT_HUMANO_MINUTOS
import requests
...
def _process_webhook(data):
    """Processa evento webhook do WhatsApp."""
    try:
        ...
        central_instance = EVOLUTION_INSTANCE_NAME
        ...
        # FLUXO CENTRAL vs FLUXO CLIENTE
        if nome_instancia_atual == central_instance:
            _processar_instancia_central(phone_number, message_text, msg_clean, is_from_me, agora, nome_instancia_atual, document_message, db)
        else:
            _processar_instancia_cliente(phone_number, message_text, msg_clean, is_from_me, agora, nome_instancia_atual, document_message, db)

    except Exception as e:
        erro_completo = f"Erro Webhook (Instância: {data.get('instance')}): {e}"
        logger.error(f"❌ {erro_completo}")
        notificar_erro_admin(erro_completo)

def _processar_instancia_central(phone_number, message_text, msg_clean, is_from_me, agora, nome_instancia_atual, document_message, db):
    """Processa mensagens na instância central (Suporte e Vendas)."""
    ...
    if msg_clean == "#qrcode":
        send_whatsapp(phone_number, "🔄 A gerar QR Code...", instance_name=nome_instancia_atual)
    ...
    if any(g in msg_clean for g in gatilhos_teste):
        ...
        if status_atual == 'prospect':
            send_whatsapp(phone_number, "⏳ *A preparar o seu teste de 2 dias...* 🚀", instance_name=nome_instancia_atual)
...
ATENÇÃO: Nunca ignore novas saudações (como 'Bom dia', 'Boa noite', 'Oy'). Retome a conversa de forma natural, prestativa e ativa."""
...
def _processar_instancia_cliente(phone_number, message_text, msg_clean, is_from_me, agora, nome_instancia_atual, document_message, db):
    """Processa mensagens na instância do cliente (Bot do utilizador final)."""
    ...
    if msg_clean.startswith("/criar-arte"):
        ...
        requests.post(f"{EVOLUTION_API_URL}/message/sendMedia/{nome_instancia_atual}", headers={"apikey": EVOLUTION_API_KEY, "Content-Type": "application/json"}, json=payload, timeout=25)
        return
